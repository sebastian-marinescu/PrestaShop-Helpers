...
4. **Dynamic Git Alignment**: Dynamically detects the currently active branch on Staging (e.g., `staging`, `feature/image-grid`, or `master`) and aligns it with its remote counterpart, preserving your active workspace branch instead of forcing a checkout to `master`.
5. **Database Replication & Dual Backups**: Automatically creates safety database backups for **BOTH Production** (`backup_prod_before_sync_*.sql`) and **Staging** (`backup_staging_before_sync_*.sql`) in `helper/backups/` before performing any database operations. Dumps Production with automatic optimization (skipping heavy data for logs & statistics while keeping table structures), imports it into Staging, and updates shop URLs (`ps_shop_url` and `ps_configuration`), with options to override the shop email (`PS_SHOP_EMAIL`) and automatically disable production-only modules (like `ps_accounts`). Keeps the 3 newest backups for both environments and cleans up older ones (>7 days).
...
* `DEVELOPER_EMAIL=dev@domain.com` (change this to your email to override the shop email address on Staging)
* `DISABLE_MODULES_ON_STAGING="ps_accounts"` (space-separated list of modules to disable on Staging)
